  ...
  <main class="container mx-auto flex-1 py-16 px-4 relative z-10 max-w-7xl">
    ...
    {:else}
      <div class="grid grid-cols-1 grid-rows-1 mb-12">
        ...
        <!-- Global container for all preloaded previews -->
        {#if showPreviews && !isGridView}
          <div bind:this={previewsContainer} class="fixed z-50 pointer-events-none">
            {#each sortedAndFilteredRoutes as route, i}
              <div
                class="w-72 h-40 absolute top-0 left-0"
                style="
                  opacity: {hoveredRouteIndex === i ? 1 : 0}; 
                  visibility: {hoveredRouteIndex === i ? 'visible' : 'hidden'};
                  transform: translate({mouseX + 20}px, {mouseY - 80}px);
