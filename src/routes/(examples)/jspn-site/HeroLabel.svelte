...
<div class="relative {className}" style="--color:{color}" data-error={imgError}>
  ...
  {#if !imgError}
    <div class="{position == 'top-left' || position == 'bottom-right' ? `rounded-tl-${rounded} rounded-br-${rounded} ` : `rounded-tr-${rounded} rounded-bl-${rounded} `}  label-container label-{position} label-container label-{position} z-10" style="background-color:var(--color)">
...
{#snippet corner({ edge, alignment, rotation }: CornerConfig)}
  <div class="absolute size-2 {edge === 'top' && alignment === 'start' ? 'top-start -top-2 left-0' : edge === 'top' && alignment === 'end' ? 'top-end -top-2 right-0' : edge === 'right' && alignment === 'start' ? 'right-start top-0 -right-2 ' : edge === 'right' && alignment === 'end' ? 'right-end bottom-0 -right-2' : edge === 'bottom' && alignment === 'start' ? 'bottom-start -bottom-2 left-0' : edge === 'bottom' && alignment === 'end' ? 'bottom-end -bottom-2 right-0' : edge === 'left' && alignment === 'start' ? 'left-start top-0 -left-2' : 'left-end -left-2 bottom-0'}">
